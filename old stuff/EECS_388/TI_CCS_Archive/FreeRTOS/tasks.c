...
/*--GJM: Deleted "static" so that we see these variables in the LinkMap
 *       "PRIVILEGED_DATA" is NULL if MPU is not enabled
 */
...
PRIVILEGED_DATA extern volatile UBaseType_t uxCurrentNumberOfTasks 	= ( UBaseType_t ) 0U;
PRIVILEGED_DATA extern volatile TickType_t xTickCount 				= ( TickType_t ) 0U;
PRIVILEGED_DATA extern volatile UBaseType_t uxTopReadyPriority 		= tskIDLE_PRIORITY;
PRIVILEGED_DATA extern volatile BaseType_t xSchedulerRunning 		= pdFALSE;
PRIVILEGED_DATA extern volatile UBaseType_t uxPendedTicks 			= ( UBaseType_t ) 0U;
PRIVILEGED_DATA extern volatile BaseType_t xYieldPending 			= pdFALSE;
PRIVILEGED_DATA extern volatile BaseType_t xNumOfOverflows 			= ( BaseType_t ) 0;
PRIVILEGED_DATA extern volatile UBaseType_t uxTaskNumber 					= ( UBaseType_t ) 0U;
PRIVILEGED_DATA extern volatile TickType_t xNextTaskUnblockTime		= portMAX_DELAY;
