...
struct sem_rec *call(char *f, struct sem_rec *args)
{
  ...
  struct sem_rec* sr_array[num];
  temp = args;

  for (int i=0; i<num; i++)
  {
    sr_array[i] = temp;
    temp = temp->back.s_true;
  }

  for (int j=0; j<num; j++)
  {
    if(sr_array[j]->s_mode == T_DOUBLE)
    {
      printf("argf t%i\n", sr_array[j]->s_place);
    }
    else
  ...
  struct id_entry* temp2;
  if ( (temp2 = lookup(f, 0)) == NULL )
  {
    temp2 = install(f,0);
    temp2->i_type = T_INT;
    temp2->i_scope = GLOBAL;
    temp2->i_defined = 1;
  }

  char* s;
  if(temp2->i_scope == GLOBAL)
  {
    s = "global";
  }
  else if (temp2->i_scope == LOCAL)
  {
    s = "local";
  }
  else if (temp2->i_scope == PARAM)
  {
    s = "param";
  }

  printf("t%d := %s %s\n", nexttemp(), s, f);
  printf("t%d := f%d t%d %d\n", nexttemp(), temp2->i_type, currtemp(), num);
...
struct id_entry *fname(int t, char *id)
{
   ...
   struct id_entry* temp = install(id,0);
   temp->i_scope = GLOBAL;
   temp->i_type = t;
   temp->i_defined = 1;
...
struct sem_rec *op2(char *op, struct sem_rec *x, struct sem_rec *y)
{
   ...
   return ( gen(op, cast(x,a), cast(y,a), a) );
}
...
struct sem_rec *opb(char *op, struct sem_rec *x, struct sem_rec *y)
{
  ...
  {
    fprintf(stderr, "Types dont match: opb func\n");
  }
}
...
struct sem_rec *rel(char *op, struct sem_rec *x, struct sem_rec *y)
{
  ...
  struct sem_rec* temp = gen(op, cast(x,a), cast(y,a), a);
...
struct sem_rec *set(char *op, struct sem_rec *x, struct sem_rec *y)
{
  ...
  {
    p = gen("@", (struct sem_rec*) NULL, x, x->s_mode);
    if (y->s_mode == T_ARRAY)
    {
      cast_y = gen("@", (struct sem_rec*) NULL, cast(y, x->s_mode), y->s_mode);
    }
    else
    {
      cast_y = cast(y, x->s_mode);
    }

    struct sem_rec* temp = gen(op, p, cast_y, x->s_mode);
    return ( gen("=", x, temp, x->s_mode) );
  }
...
struct sem_rec *string(char *s)
{
  ...
  struct sem_rec* temp;
  temp = (struct sem_rec *)malloc(sizeof(struct sem_rec));
  temp -> s_place = currtemp();
